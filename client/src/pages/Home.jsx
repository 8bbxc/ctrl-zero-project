...
export default function Home() {
  ...
  return (
    <div ref={containerRef} className="bg-[#0f172a] text-slate-50 font-sans selection:bg-cyan-500/30 overflow-hidden">
      
      {/* 1. HERO SECTION (More Vibrant) */}
      <section className="relative min-h-screen flex items-center justify-center pt-20 pb-12 overflow-hidden">
        {/* خلفية ملونة أكثر */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-[-20%] left-[-10%] w-[1000px] h-[1000px] bg-blue-600/20 rounded-full blur-[120px] animate-pulse-slow" />
          <div className="absolute bottom-[-20%] right-[-10%] w-[1000px] h-[1000px] bg-purple-600/20 rounded-full blur-[120px] animate-pulse-slow" style={{ animationDelay: '2s' }} />
        ...
        >
            <motion.div 
              ...
              className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-cyan-500/30 bg-cyan-900/20 backdrop-blur-xl mb-8 hover:bg-cyan-900/30 transition-colors cursor-default"
            >
            ...
            <motion.div 
              ...
              className="text-lg md:text-2xl text-slate-300 font-light mb-10 h-10 flex items-center justify-center gap-2"
            >
            ...
            >
              <Link to="/projects" className="group relative px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-full font-bold text-lg overflow-hidden transition-all hover:scale-105 shadow-lg shadow-cyan-500/25">
      ...
      <section className="py-24 bg-[#0f172a] relative overflow-hidden z-20">
        <div className="transform -rotate-2 scale-110 origin-center">
          <div className="bg-[#1e293b]/50 border-y border-white/5 py-10 shadow-2xl relative backdrop-blur-sm">
      ...
      <section className="py-24 container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
           ...
           <p className="text-slate-400">A streamlined process to deliver excellence.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
           {processSteps.map((step, idx) => (
             <motion.div 
               ...
               viewport={{ once: true }}
               className="p-6 rounded-2xl bg-slate-800/50 border border-slate-700 hover:border-cyan-500/50 transition-all hover:-translate-y-2 group"
             >
                <div className="w-14 h-14 rounded-full bg-slate-900 flex items-center justify-center text-2xl text-cyan-400 mb-6 group-hover:scale-110 transition-transform shadow-lg shadow-cyan-900/20">
                  {step.icon}
                </div>
      ...
      <section className="py-16 container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {/* Card 1: Full Stack */}
          <motion.div whileHover={{ y: -5 }} className="md:col-span-2 p-10 rounded-[2rem] bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700/50 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-[300px] h-[300px] bg-blue-500/10 rounded-full blur-[80px] -z-10 group-hover:bg-blue-500/20 transition-all" />
            <div className="mb-6 w-14 h-14 rounded-xl bg-blue-500/20 flex items-center justify-center text-blue-400 text-3xl"><FaLaptopCode /></div>
            <h3 className="text-2xl font-bold text-white mb-3">{t('services.fullstack.title') || 'Full-Stack Engineering'}</h3>
            <p className="text-slate-300 mb-6">{t('services.fullstack.desc') || 'Robust web applications tailored for scale.'}</p>
            <Link to="/services/web-dev" className="text-blue-400 font-bold hover:text-blue-300 flex items-center gap-2">Learn more <FaArrowRight /></Link>
          </motion.div>

          {/* Card 2: UI/UX */}
          <motion.div whileHover={{ y: -5 }} className="md:row-span-2 p-10 rounded-[2rem] bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700/50 relative overflow-hidden group">
             <div className="absolute bottom-0 left-0 w-[200px] h-[200px] bg-purple-500/10 rounded-full blur-[80px] -z-10 group-hover:bg-purple-500/20 transition-all" />
             <div className="mb-6 w-14 h-14 rounded-xl bg-purple-500/20 flex items-center justify-center text-purple-400 text-3xl"><FaCode /></div>
             <h3 className="text-2xl font-bold text-white mb-3">{t('services.uiux.title') || 'UI/UX Design'}</h3>
             <p className="text-slate-300">{t('services.uiux.desc') || 'Interfaces that users love to use.'}</p>
          </motion.div>

          {/* Card 3: Product */}
          <motion.div whileHover={{ y: -5 }} className="p-10 rounded-[2rem] bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700/50 relative overflow-hidden group">
             <div className="mb-6 w-12 h-12 rounded-xl bg-orange-500/20 flex items-center justify-center text-orange-400 text-2xl"><FaNodeJs /></div>
             <h3 className="text-xl font-bold text-white mb-2">{t('services.product.title') || 'Product Eng.'}</h3>
             <p className="text-slate-400 text-sm">{t('services.product.desc')}</p>
          </motion.div>

           {/* Card 4: DevOps */}
           <motion.div whileHover={{ y: -5 }} className="p-10 rounded-[2rem] bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700/50 relative overflow-hidden group">
             <div className="mb-6 w-12 h-12 rounded-xl bg-emerald-500/20 flex items-center justify-center text-emerald-400 text-2xl"><FaDocker /></div>
             <h3 className="text-xl font-bold text-white mb-2">{t('services.devops.title') || 'Cloud & DevOps'}</h3>
             <p className="text-slate-400 text-sm">{t('services.devops.desc')}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 bg-[#0b1120] relative border-y border-white/5">
        <div className="container mx-auto px-4">
          <h2 className="text-center text-3xl font-bold text-white mb-12">Trusted by Visionaries</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((testi, i) => (
              <motion.div 
                ...
                className="p-8 rounded-2xl bg-[#151e32] border border-slate-700/50 relative"
              >
                <FaQuoteLeft className="text-cyan-500/20 text-4xl absolute top-6 left-6" />
                <p className="text-slate-300 leading-relaxed mb-6 relative z-10">"{testi.text}"</p>
                <div>
      ...
      {/* 7. BIG CTA */}
      <section className="relative py-40 overflow-hidden text-center bg-[#050914]">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <h2 className="text-5xl md:text-7xl lg:text-8xl font-black text-white mb-8 tracking-tight">
            LET'S BUILD <br className="hidden md:block"/> THE <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500">EXTRAORDINARY.</span>
          </h2>
          <Link 
            to="/contact" 
