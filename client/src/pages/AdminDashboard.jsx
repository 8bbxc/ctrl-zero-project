...
const StatCard = ({ title, count, icon, color, delay }) => (
  <motion.div 
    ...
    className="relative overflow-hidden bg-slate-900/60 backdrop-blur-xl border border-white/5 p-3 sm:p-4 md:p-6 rounded-xl sm:rounded-2xl md:rounded-3xl group hover:border-white/10 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/10"
  >
    <div className={`absolute -top-4 -right-4 p-4 opacity-5 md:opacity-10 group-hover:opacity-20 transition-opacity ${color} text-5xl sm:text-6xl md:text-8xl rotate-12`}>
      {icon}
    </div>
    <div className="relative z-10">
      <div className={`w-7 sm:w-8 md:w-10 h-7 sm:h-8 md:h-10 mb-2 md:mb-4 rounded-lg md:rounded-xl flex items-center justify-center bg-white/5 ${color} bg-clip-text text-base sm:text-lg md:text-2xl`}>
        {icon}
      </div>
      <h3 className="text-slate-400 text-[9px] sm:text-[10px] md:text-xs font-bold uppercase tracking-widest mb-0.5 md:mb-1">{title}</h3>
      <p className="text-xl sm:text-2xl md:text-4xl font-black text-white tracking-tight">{count}</p>
    </div>
  </motion.div>
)
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 font-sans selection:bg-accent selection:text-black overflow-x-hidden flex flex-col">
      ...
      <div className="flex flex-1 h-[calc(100vh-80px)] overflow-hidden pt-20"> {/* Added pt-20 for Navbar spacing */}
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 h-20 bg-slate-900/60 backdrop-blur-xl border-t border-white/5 flex items-center justify-around z-40">
          {[
            { id: 'projects', icon: <FaProjectDiagram className="text-base md:text-xl" />, label: 'Projects' },
            { id: 'services', icon: <FaLayerGroup className="text-base md:text-xl" />, label: 'Services' },
            { id: 'messages', icon: <FaEnvelope className="text-base md:text-xl" />, label: 'Messages' }
          ].map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex-1 h-full flex flex-col items-center justify-center transition-all relative gap-0.5 group`}
              title={tab.label}
            >
              <div className={`transition-all ${activeTab === tab.id ? 'text-accent scale-110' : 'text-slate-500 group-hover:text-white'}`}>
                {tab.icon}
              </div>
              <span className={`text-[8px] font-bold transition-all ${activeTab === tab.id ? 'text-accent' : 'text-slate-500 group-hover:text-slate-300'}`}>{tab.label.slice(0,3)}</span>
        ...
        {/* Content */}
        <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-gradient-to-br from-transparent to-slate-900/30 md:pb-0 pb-24">
          
          <header className="h-auto px-3 sm:px-4 md:px-10 py-3 sm:py-4 md:py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between z-30 gap-3 sm:gap-4 md:gap-6 flex-wrap">
            <div>
              <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-white tracking-tight">Dashboard</h1>
              <p className="text-[10px] sm:text-xs md:text-sm text-slate-500 font-mono mt-1 flex items-center gap-1.5 sm:gap-2">
                <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-500 animate-pulse"/> SYSTEM ONLINE
              </p>
            </div>

            <div className="flex flex-col xs:flex-row items-stretch gap-2 sm:gap-3 md:gap-6 w-full sm:w-auto">
              <div className="relative group flex-1 xs:flex-none">
                <FaSearch className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-accent transition-colors text-sm sm:text-base" />
                <input 
                  type="text" 
                  placeholder="Search..." 
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full sm:w-56 md:w-72 bg-slate-950/60 border border-white/10 rounded-lg sm:rounded-xl md:rounded-2xl pl-9 sm:pl-10 md:pl-12 pr-3 sm:pr-4 md:pr-6 py-2 sm:py-2.5 md:py-3 text-xs sm:text-sm focus:w-full md:focus:w-96 transition-all outline-none focus:border-accent/50 text-white placeholder-slate-600 shadow-inner"
                />
              </div>
              
              {activeTab !== 'messages' && (
                <button onClick={() => openModal()} className="btn-primary flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-6 md:px-8 py-2 sm:py-2.5 md:py-3 rounded-lg sm:rounded-xl md:rounded-2xl text-[11px] sm:text-xs md:text-sm font-bold shadow-lg hover:-translate-y-1 transition-all whitespace-nowrap flex-shrink-0">
                  <FaPlus className="text-sm sm:text-base" /> <span className="hidden sm:inline">New</span>
                </button>
              )}
            </div>
          </header>

          <div className="flex-1 overflow-y-auto px-2 sm:px-3 md:px-10 pb-10 md:pb-10 custom-scrollbar">
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 md:gap-8 mb-6 sm:mb-8 md:mb-12">
              <StatCard title="Projects" count={counts.projects} icon={<FaProjectDiagram />} color="text-blue-500" delay={0.1} />
              <StatCard title="Services" count={counts.services} icon={<FaLayerGroup />} color="text-purple-500" delay={0.2} />
              <StatCard title="Messages" count={counts.messages} icon={<FaEnvelope />} color="text-emerald-500" delay={0.3} />
            </div>

            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1.5 sm:gap-2 md:gap-4 mb-4 sm:mb-6 md:mb-8">
              <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white capitalize">{activeTab}</h2>
              <div className="hidden sm:block h-px flex-1 bg-gradient-to-r from-white/10 to-transparent" />
              <span className="text-[9px] sm:text-[10px] md:text-xs font-mono text-slate-500">{filteredItems.length} ITEMS</span>
            </div>
            ...
            {loading ? (
              ...
              <motion.div layout className="min-h-[200px]">
                {activeTab === 'messages' ? (
                  ...
                  <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-4 md:gap-6 lg:gap-8">
                    <AnimatePresence>
                      {filteredItems.map((item, i) => (
                        <motion.div 
                          ...
                          transition={{ delay: i * 0.05 }}
                          className="group relative bg-slate-900/40 backdrop-blur-sm border border-white/5 rounded-2xl sm:rounded-3xl overflow-hidden hover:border-accent/30 hover:shadow-2xl hover:shadow-accent/5 transition-all duration-500 flex flex-col h-full"
                        >
                          <div className="h-24 xs:h-32 sm:h-40 md:h-48 overflow-hidden relative bg-slate-950 flex-shrink-0">
                             <img 
                             ...
                             <div className="absolute top-1 right-1 xs:top-1.5 xs:right-1.5 sm:top-2 sm:right-2 md:top-3 md:right-3 flex gap-1 xs:gap-1.5 md:gap-2 translate-y-[-10px] opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                               <button onClick={() => openModal(item)} className="p-1 xs:p-1.5 md:p-2 bg-white/10 backdrop-blur-md rounded md:rounded-xl text-xs xs:text-sm md:text-base text-white hover:bg-accent hover:text-black transition-colors shadow-lg"><FaEdit /></button>
                               <button onClick={() => setConfirmDelete({ open: true, id: item.id })} className="p-1 xs:p-1.5 md:p-2 bg-black/40 backdrop-blur-md rounded md:rounded-xl text-xs xs:text-sm md:text-base text-red-400 hover:bg-red-500 hover:text-white transition-colors shadow-lg"><FaTrash /></button>
                             </div>
                             <div className="absolute top-1 left-1 xs:top-1.5 xs:left-1.5 sm:top-2 sm:left-2 md:top-3 md:left-3">
                               <span className="bg-black/50 backdrop-blur-md text-[7px] xs:text-[8px] md:text-[10px] font-bold px-1 xs:px-1.5 md:px-2 py-0.5 md:py-1 rounded text-white border border-white/5">ID: {item.id}</span>
                             </div>
                          </div>
                          <div className="p-2 xs:p-3 md:p-5 relative z-20 flex-1 flex flex-col">
                            ...
                            <div className="flex items-center gap-0.5 xs:gap-1 md:gap-2 pt-1.5 xs:pt-2 md:pt-3 border-t border-white/5 text-[8px] xs:text-[9px] md:text-[10px] flex-wrap">
                              {item.gallery?.length > 0 && (
                                <span className="bg-white/5 px-1 xs:px-1.5 md:px-2 py-0.5 md:py-1 rounded text-slate-400 flex items-center gap-0.5 xs:gap-0.5 md:gap-1 whitespace-nowrap"><FaImages className="text-[7px] xs:text-[8px] md:text-xs" /> {item.gallery.length}</span>
                              )}
                              {item.tags && Array.isArray(item.tags) && item.tags.slice(0,1).map((t, idx) => (
                                <span key={idx} className="bg-accent/5 text-accent px-1 xs:px-1.5 md:px-2 py-0.5 md:py-1 rounded border border-accent/10 truncate text-[7px] xs:text-[8px]">#{t}</span>
                              ))}
                            </div>
                          </div>
                        </motion.div>
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 100, x: 0 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 100 }} transition={{ type: "spring", duration: 0.4 }} className="relative w-full max-h-[98vh] md:max-h-[90vh] md:max-w-4xl bg-gradient-to-b from-slate-900 to-slate-950 border-t md:border border-white/10 rounded-t-2xl md:rounded-3xl shadow-2xl overflow-hidden flex flex-col mx-0 md:mx-4">
              <div className="sticky top-0 px-3 sm:px-6 md:px-8 py-3 sm:py-4 md:py-6 border-b border-white/5 flex justify-between items-center bg-slate-900/95 backdrop-blur-sm z-10">
                <h3 className="text-sm sm:text-base md:text-xl font-bold text-white tracking-tight flex items-center gap-1 sm:gap-2">
                  <span className="w-1.5 h-4 sm:h-5 md:h-6 bg-accent rounded-full mr-1 md:mr-2"/>
                  <span className="text-xs sm:text-sm md:text-lg truncate">{editingItem ? `Edit ${activeTab.slice(0,-1)}` : `Create New ${activeTab.slice(0,-1)}`}</span>
                </h3>
                <button onClick={() => setShowModal(false)} className="flex-shrink-0 w-8 sm:w-10 h-8 sm:h-10 rounded-full flex items-center justify-center bg-white/5 text-slate-400 hover:bg-red-500 hover:text-white transition"><FaTimes className="text-sm sm:text-base" /></button>
              </div>
              <div className="flex-1 overflow-y-auto custom-scrollbar">
                <form id="itemForm" onSubmit={handleSave} className="space-y-3 sm:space-y-6 md:space-y-10 p-3 sm:p-6 md:p-8">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-6 md:gap-8">\n                      <div className="md:col-span-2 space-y-3 sm:space-y-6 md:space-y-6">
                         ...
                         {activeTab === 'projects' && (
                            <div className="space-y-1.5 md:space-y-2 w-full">
                              <label className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">Project Sector</label>
                              <select 
                                value={formData.category || 'General'}
                                onChange={e => setFormData({...formData, category: e.target.value})}
                                className="w-full bg-black/40 border border-white/10 rounded-lg md:rounded-xl px-3 md:px-4 py-2.5 md:py-3 text-sm text-white focus:border-accent focus:bg-black/60 focus:ring-1 focus:ring-accent/30 outline-none transition-all cursor-pointer"
                              >
                         ...
                         <InputGroup label="Short Description" isTextArea value={formData.description || formData.shortDescription} onChange={e => setFormData({...formData, [activeTab === 'projects' ? 'description' : 'shortDescription']: e.target.value})} placeholder="Brief summary... (1-2 sentences)" />
                      </div>
                  ...
                  <div className="space-y-4 md:space-y-6 pt-4 md:pt-6 border-t border-white/5">
                      ...
                      {activeTab === 'projects' && (
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 pt-2 md:pt-4">
              ...
              <div className="sticky bottom-0 p-3 sm:p-4 md:p-6 border-t border-white/5 bg-slate-900 flex flex-col-reverse sm:flex-row justify-end gap-2 md:gap-4 z-20 w-full">
                <button type="button" onClick={() => setShowModal(false)} className="w-full sm:w-auto px-3 sm:px-6 md:px-6 py-2.5 sm:py-3 md:py-3 rounded-lg md:rounded-xl text-slate-300 hover:text-white hover:bg-white/5 transition font-bold text-xs sm:text-sm">
                  Cancel
                </button>
                <button type="submit" form="itemForm" className="w-full md:w-auto btn-primary px-6 md:px-10 py-3 md:py-3 rounded-lg md:rounded-xl shadow-xl shadow-accent/20 hover:shadow-accent/40 font-bold text-sm tracking-wide">
                  {editingItem ? 'SAVE' : 'PUBLISH'}
                </button>
              </div>
            </motion.div>
          </div>
...
const InputGroup = ({ label, value, onChange, required, placeholder, isTextArea }) => (
  <div className="space-y-1 sm:space-y-1.5 md:space-y-2 w-full">
    <label className="text-[9px] sm:text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">{label}</label>
    {isTextArea ? (
       <textarea rows="3" className="w-full bg-black/40 border border-white/10 rounded-lg md:rounded-xl px-2.5 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-3 text-xs sm:text-sm text-white focus:border-accent focus:bg-black/60 focus:ring-1 focus:ring-accent/30 outline-none transition-all placeholder-slate-600 resize-none" value={value || ''} onChange={onChange} required={required} placeholder={placeholder} />
    ) : (
       <input className="w-full bg-black/40 border border-white/10 rounded-lg md:rounded-xl px-2.5 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-3 text-xs sm:text-sm text-white focus:border-accent focus:bg-black/60 focus:ring-1 focus:ring-accent/30 outline-none transition-all placeholder-slate-600" value={value || ''} onChange={onChange} required={required} placeholder={placeholder} />
    )}
  </div>
)

const UploadBox = ({ label, loading, onChange, multiple, preview, isGallery, gallery, onRemove, onRemoveGallery, isIcon }) => (
  <div className="space-y-1 sm:space-y-1.5 md:space-y-2 h-full flex flex-col">
    <div className="flex justify-between items-center px-1">
       <label className="text-[9px] sm:text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider">{label}</label>
       {loading && <span className="text-[9px] sm:text-[10px] md:text-xs text-accent animate-pulse font-bold">UPLOADING...</span>}
    </div>
    {!isGallery ? (
       <div className={`relative flex-1 border-2 border-dashed border-white/10 rounded-lg md:rounded-xl overflow-hidden hover:border-accent/40 transition-all group bg-black/30 min-h-[90px] xs:min-h-[100px] md:min-h-[150px] ${preview ? 'border-solid border-white/5' : ''}`}>
          ...
          {preview ? (
             <div className="absolute inset-0 z-10">
                <img src={preview} alt="Preview" className={`w-full h-full ${isIcon ? 'object-contain p-4 sm:p-6 md:p-8' : 'object-cover'}`} loading="lazy" />
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-0.5 md:gap-2">
                   <span className="text-white text-[9px] sm:text-xs font-bold">Change Image</span>
                   <button onClick={(e) => { e.preventDefault(); onRemove?.() }} className="px-1.5 sm:px-2 md:px-3 py-0.5 md:py-1 bg-red-500/80 hover:bg-red-500 text-white text-[8px] md:text-xs rounded z-30 relative">Remove</button>
                </div>
             </div>
          ) : (
             <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <div className="w-9 sm:w-10 md:w-12 h-9 sm:h-10 md:h-12 rounded-full bg-slate-800 flex items-center justify-center mb-1 sm:mb-1.5 md:mb-2 group-hover:scale-110 transition-transform"><FaCloudUploadAlt className="text-base sm:text-lg md:text-xl text-slate-400" /></div>
                <span className="text-[9px] sm:text-[10px] md:text-xs text-slate-500 font-bold text-center px-1.5 sm:px-2">Drop image here</span>
             </div>
          )}
       </div>
    ) : (
       <div className="space-y-1.5 sm:space-y-2 md:space-y-3">
          <div className="relative border-2 border-dashed border-white/10 rounded-lg md:rounded-xl p-3 sm:p-4 md:p-6 text-center hover:border-accent/40 transition-all bg-black/30 group cursor-pointer">
             ...
             <div className="pointer-events-none">
                <FaImages className="mx-auto text-lg sm:text-xl md:text-2xl text-slate-500 mb-1 sm:mb-1.5 md:mb-3 group-hover:text-white transition-colors" />
                <span className="text-[9px] sm:text-[10px] md:text-xs text-slate-400 font-bold block">Add Images to Gallery</span>
             </div>
          </div>
          {gallery?.length > 0 && (
             <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-4 gap-1 sm:gap-2 md:gap-3">
                {gallery.map((img, i) => (
                   <div key={i} className="relative aspect-square rounded-lg overflow-hidden border border-white/10 group">
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                         <button type="button" onClick={() => onRemoveGallery?.(i, img.public_id)} className="w-5 sm:w-6 md:w-8 h-5 sm:h-6 md:h-8 bg-red-500 text-white rounded-full flex items-center justify-center hover:scale-110 transition-transform"><FaTrash size={10} /></button>
                      </div>
                   </div>
                ))}
             </div>
