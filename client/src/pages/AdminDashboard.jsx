...
const StatCard = ({ title, count, icon, color, delay }) => (
  <motion.div 
    ...
    className="relative overflow-hidden bg-slate-900/60 backdrop-blur-xl border border-white/5 p-6 rounded-3xl group hover:border-white/10 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/10"
  >
    <div className={`absolute -top-4 -right-4 p-4 opacity-10 group-hover:opacity-20 transition-opacity ${color} text-8xl rotate-12`}>
      {icon}
    </div>
    <div className="relative z-10">
      <div className={`w-10 h-10 mb-4 rounded-xl flex items-center justify-center bg-white/5 ${color} bg-clip-text`}>
        {icon}
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 font-sans selection:bg-accent selection:text-black overflow-x-hidden flex flex-col">
      ...
      <div className="flex flex-1 h-[calc(100vh-80px)] overflow-hidden pt-20"> {/* Added pt-20 for Navbar spacing */}
        ...
        {/* Content */}
        <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-gradient-to-br from-transparent to-slate-900/30">
            ...
            <div className="flex items-center gap-6">
              <div className="relative group">
                ...
                <input 
                  ...
                  className="bg-slate-950/60 border border-white/10 rounded-2xl pl-12 pr-6 py-3 text-sm w-72 focus:w-96 transition-all outline-none focus:border-accent/50 text-white placeholder-slate-600 shadow-inner"
                />
              </div>
              
              {activeTab !== 'messages' && (
                <button onClick={() => openModal()} className="btn-primary flex items-center gap-2 px-8 py-3 rounded-2xl text-sm font-bold shadow-lg hover:-translate-y-1 transition-all">
          ...
          <div className="flex-1 overflow-y-auto px-10 pb-10 custom-scrollbar">
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <span className="text-xs font-mono text-slate-500">{filteredItems.length} ITEMS FOUND</span>
            </div>
                    ...
                    <AnimatePresence>
                      {filteredItems.map((msg, i) => (
                        <motion.div 
                          ...
                          className="bg-slate-900/40 border border-white/5 rounded-2xl p-6 hover:bg-slate-800/40 hover:border-white/10 transition-all group relative overflow-hidden"
                        >
                          <div className={`absolute left-0 top-0 bottom-0 w-1 ${msg.read ? 'bg-slate-700' : 'bg-accent'}`} />
                          <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center text-slate-300 border border-white/5 shadow-inner">
                                <FaUser className="text-lg" />
                              </div>
                              <div className="flex flex-col">
                                <h3 className="font-bold text-white text-lg flex items-center gap-2">
                                  {msg.name || "Unknown Sender"}
                                  {!msg.read && <span className="bg-accent text-black text-[10px] px-2 rounded-full shadow-lg shadow-accent/20">NEW</span>}
                                </h3>
                                <p className="text-sm text-slate-300 font-medium">{msg.subject || "No Subject"}</p>
                                <p className="text-xs text-slate-500 font-mono mt-1 flex items-center gap-2"><FaEnvelope className="text-accent/70" /> {msg.email}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-4">
                              <span className="text-xs text-slate-500 flex items-center gap-1 bg-black/20 px-3 py-1.5 rounded-lg border border-white/5"><FaClock /> {new Date(msg.date || msg.createdAt).toLocaleDateString()}</span>
                          ...
                          <div className="pl-16">
                             <p className="text-slate-300 text-sm leading-relaxed bg-black/20 p-5 rounded-2xl border border-white/5 font-light">{msg.fullContent || msg.message}</p>
                          </div>
                        </motion.div>
                      ))}
                    </AnimatePresence>
                        ...
                        >
                          <div className="h-48 overflow-hidden relative bg-slate-950">
                             ...
                             <div className="absolute top-3 left-3">
                               <span className="bg-black/50 backdrop-blur-md text-[10px] font-bold px-2 py-1 rounded-lg text-white border border-white/5">ID: {item.id}</span>
                             </div>
                          </div>
                          <div className="p-5 relative z-20">
                            <h3 className="font-bold text-white text-lg mb-1 truncate">{item.title}</h3>
                            <p className="text-xs text-slate-400 line-clamp-2 leading-relaxed mb-4 min-h-[2.5em]">{item.shortDescription || item.description || "No description."}</p>
                            <div className="flex items-center gap-2 pt-3 border-t border-white/5">
                              {item.gallery?.length > 0 && (
                                <span className="text-[10px] bg-white/5 px-2 py-1 rounded text-slate-400 flex items-center gap-1"><FaImages /> {item.gallery.length}</span>
                              )}
                              {item.tags && Array.isArray(item.tags) && item.tags.slice(0,2).map((t, idx) => (
                                <span key={idx} className="text-[10px] bg-accent/5 text-accent px-2 py-1 rounded border border-accent/10">#{t}</span>
                              ))}
                            </div>
                          </div>
                        </motion.div>
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 50 }} transition={{ type: "spring", duration: 0.5 }} className="relative w-full max-w-4xl bg-slate-900 border border-white/10 rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
              <div className="px-8 py-6 border-b border-white/5 flex justify-between items-center bg-slate-900 z-10">
                <h3 className="text-xl font-bold text-white tracking-tight flex items-center gap-2">
                  <span className="w-2 h-6 bg-accent rounded-full mr-2"/>
              ...
              <div className="p-8 overflow-y-auto custom-scrollbar bg-gradient-to-b from-slate-900 to-slate-950">
                <form id="itemForm" onSubmit={handleSave} className="space-y-10">
                  ...
                  <div className="space-y-6 pt-6 border-t border-white/5">
                      ...
                      <div className="space-y-2">
                         ...
                         <textarea rows="8" className="w-full bg-black/20 border border-white/10 rounded-2xl px-5 py-4 text-white focus:border-accent outline-none font-mono text-sm leading-relaxed" value={formData.content || formData.fullContent || ''} onChange={e => setFormData({...formData, [activeTab === 'projects' ? 'content' : 'fullContent']: e.target.value})} placeholder="# Write your content here..." />
                      </div>
                      {activeTab === 'services' ? (
              ...
              <div className="p-6 border-t border-white/5 bg-slate-900 flex justify-end gap-4 z-20">
                <button type="button" onClick={() => setShowModal(false)} className="px-6 py-3 rounded-xl text-slate-400 hover:text-white transition font-bold text-sm">Cancel</button>
                <button type="submit" form="itemForm" className="btn-primary px-10 py-3 rounded-xl shadow-xl shadow-accent/20 hover:shadow-accent/40 font-bold text-sm tracking-wide">{editingItem ? 'SAVE CHANGES' : 'PUBLISH NOW'}</button>
              </div>
            </motion.div>
          </div>
        )}
...
const InputGroup = ({ label, value, onChange, required, placeholder, isTextArea }) => (
  <div className="space-y-2 w-full">
    ...
    {isTextArea ? (
       <textarea rows="3" className="w-full bg-black/20 border border-white/10 rounded-2xl px-5 py-3 text-white focus:border-accent focus:bg-slate-900/50 outline-none transition-all placeholder-slate-700 resize-none text-sm" value={value || ''} onChange={onChange} required={required} placeholder={placeholder} />
    ) : (
       <input className="w-full bg-black/20 border border-white/10 rounded-2xl px-5 py-3 text-white focus:border-accent focus:bg-slate-900/50 outline-none transition-all placeholder-slate-700 text-sm h-12" value={value || ''} onChange={onChange} required={required} placeholder={placeholder} />
    )}
  </div>
)
...
const UploadBox = ({ label, loading, onChange, multiple, preview, isGallery, gallery, onRemove, onRemoveGallery, isIcon }) => (
  <div className="space-y-2 h-full flex flex-col">
    <div className="flex justify-between items-center px-1">
       ...
       {loading && <span className="text-[10px] text-accent animate-pulse font-bold">UPLOADING...</span>}
    </div>
    {!isGallery ? (
       <div className={`relative flex-1 border-2 border-dashed border-white/10 rounded-2xl overflow-hidden hover:border-accent/40 transition-all group bg-black/20 min-h-[160px] ${preview ? 'border-solid border-white/5' : ''}`}>
          ...
          {preview ? (
             <div className="absolute inset-0 z-10">
                <img src={preview} alt="Preview" className={`w-full h-full ${isIcon ? 'object-contain p-8' : 'object-cover'}`} />
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                   <span className="text-white text-xs font-bold">Change Image</span>
                   <button onClick={(e) => { e.preventDefault(); onRemove?.() }} className="px-3 py-1 bg-red-500/80 hover:bg-red-500 text-white text-[10px] rounded-lg z-30 relative">Remove</button>
                </div>
             </div>
          ) : (
             <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-xs text-slate-500 font-bold">Drop image here</span>
             </div>
          )}
       </div>
    ) : (
       <div className="space-y-3">
          <div className="relative border-2 border-dashed border-white/10 rounded-2xl p-6 text-center hover:border-accent/40 transition-all bg-black/20 group cursor-pointer">
                ...
                {gallery.map((img, i) => (
                   <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-white/10 group">
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                         <button type="button" onClick={() => onRemoveGallery?.(i, img.public_id)} className="w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center hover:scale-110 transition-transform"><FaTrash size={12} /></button>
                      </div>
                   </div>
                ))}
             </div>
