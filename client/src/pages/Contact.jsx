...
export default function Contact() {
  ...
  return (
    <div className="min-h-screen py-24 relative overflow-hidden flex items-center bg-slate-950 text-slate-50 font-sans selection:bg-accent selection:text-black">
      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <motion.div 
            ...
            className={`space-y-10 ${dir === 'rtl' ? 'lg:order-2' : ''}`}
          >
            <div>
            ...
            {/* سوشيال ميديا */}
            <div>
              <h3 className="text-white font-bold mb-4 uppercase tracking-widest text-xs">{t('contact.social') || 'Follow Us'}</h3>
          ...
          <motion.div 
            ...
            className={`bg-slate-900/60 backdrop-blur-xl border border-white/10 p-8 md:p-10 rounded-3xl shadow-2xl relative ${dir === 'rtl' ? 'lg:order-1' : ''}`}
          >
            {/* زخرفة ضوئية */}
            ...
            <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full bg-black/20 border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-accent focus:bg-black/40 transition-all resize-none placeholder-slate-600"
                ></textarea>
              </div>
              ...
              <button 
                ...
                className="w-full py-4 rounded-xl bg-accent text-black font-bold text-lg hover:shadow-[0_0_20px_rgba(59,130,246,0.4)] hover:-translate-y-1 transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-3"
              >
                {status === 'loading' ? (
                  <> <ImSpinner8 className="animate-spin" /> {t('loading') || 'Sending...'} </>
                ) : (
                  <> <FaPaperPlane /> {t('contact.send') || 'Send Message'} </>
                )}
              </button>

              {/* رسائل الحالة */}
              {status === 'success' && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-xl text-center text-sm font-bold">
                  {t('contact.sent') || 'Message sent successfully!'}
                </motion.div>
              )}
              {status === 'error' && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl text-center text-sm font-bold">
  ...
  return (
    <div className="space-y-2">
      ...
      <input 
        ...
        className="w-full bg-black/20 border border-white/10 rounded-xl px-5 py-3.5 text-white focus:outline-none focus:border-accent focus:bg-black/40 transition-all placeholder-slate-600"
      />
    </div>
  )
}
...
function ContactCard({ icon, title, value, link, color }) {
  return (
    <a href={link || '#'} className={`flex items-center gap-5 p-5 rounded-2xl bg-slate-900/40 border border-white/5 hover:border-white/10 hover:bg-slate-800/60 transition-all group ${!link ? 'cursor-default' : ''}`}>
      <div className={`w-14 h-14 rounded-full bg-slate-800 flex items-center justify-center text-2xl ${color} shadow-lg group-hover:scale-110 transition-transform`}>
        {icon}
      </div>
      <div>
        <div className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-1">{title}</div>
        <div className="text-white font-medium text-lg group-hover:text-accent transition-colors">{value}</div>
      </div>
    </a>
  )
}
...
function SocialBtn({ icon, link, color }) {
  return (
    <a 
      ...
      target="_blank" 
      rel="noreferrer" 
      className={`w-12 h-12 rounded-full border border-white/10 bg-white/5 flex items-center justify-center text-slate-300 hover:text-white transition-all hover:scale-110 ${color}`}
    >
